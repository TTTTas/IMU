...
close all;
%% 
fid=fopen("IMU.bin",'rb');
[IMU_DATA,N]=fread(fid,'double');
IMU_DATA=reshape(IMU_DATA,7,N/7).';
fclose(fid);
%% 
fid=fopen("PureINS.bin",'rb');
[INSNAV,N]=fread(fid,'double');
INSNAV=reshape(INSNAV,10,N/10).';
fclose(fid);
%% 初始化操作
Time0=91620.0;
omiga_e= 7.292115*1e-5;
Pos0=[23.1373950708;113.3713651222;2.175];
Vel0=[0.0;0.0;0.0];
Atti0=[0.0107951084511778;-2.14251290749072;-75.7498049314083];
Start_time=find(IMU_DATA(:,1)==Time0);
Euler=rad2deg(Find_Attitude(IMU_DATA(1:Start_time-1,2:7),GRS80_g(Pos0),omiga_e,Pos0));
%% 
IMU_result=zeros(height(IMU_DATA)-Start_time,3);
...
subplot(3,1,1)
hold on
plot(IMU_result(:,1),d(:,2),'-','LineWidth',2,'DisplayName','{\Delta}B')
plot(IMU_result(:,1),d(:,3),'--','LineWidth',2,'DisplayName','{\Delta}L')
plot(IMU_result(:,1),d(:,4),'-.','LineWidth',2,'DisplayName','{\Delta}H')
title("坐标差异")
xlabel("Time/s",'FontName','Times New Roman')
ylabel("{\Delta}/deg/m",'FontName','Times New Roman')
legend
grid on
subplot(3,1,2)
hold on
plot(IMU_result(:,1),d(:,5),'-','LineWidth',2,'DisplayName','{\Delta}V_N')
plot(IMU_result(:,1),d(:,6),'--','LineWidth',2,'DisplayName','{\Delta}V_E')
plot(IMU_result(:,1),d(:,7),'-.','LineWidth',2,'DisplayName','{\Delta}V_D')
title("速度差异")
xlabel("Time/s",'FontName','Times New Roman')
ylabel("{\Delta}/m·s^{-2}",'FontName','Times New Roman')
legend
grid on
subplot(3,1,3)
hold on
plot(IMU_result(:,1),d(:,8),'-','LineWidth',2,'DisplayName','{\Delta}Roll')
plot(IMU_result(:,1),d(:,9),'--','LineWidth',2,'DisplayName','{\Delta}Pitch')
plot(IMU_result(:,1),d(:,10),'-.','LineWidth',2,'DisplayName','{\Delta}Yaw')
title("姿态差异")
xlabel("Time/s",'FontName','Times New Roman')
